...
fn test_base_path_returns_hey() -> Result<(), Error> {
    ...
    let res = client.get(BASE_URL.to_owned())
        .send()?;
...
fn test_json_path_returns_groot() -> Result<(), Error> {
    ...
    let req_msg = ReqMsg{message: "Hello Axum".to_owned()};
    ...
    assert_eq!("I am groot", &res_json.response, "Server did not groot! Expected groot.");
    Ok(())
}